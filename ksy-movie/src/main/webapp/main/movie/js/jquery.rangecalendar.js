	...
	this.each(function(i, el) {
		...
		obj._getCalendarHTML = function(startDate,endDate) {
			...
			for (var index = 1; (date.isBefore(endDate) || date.isSame(endDate)) ; index++){
                ...
                if(day == "토") {
                    cell += '<div class="cell-content sat-style">';
                    cell += '<div class="day-number sat-style">'+dayNumber+'</div>';
                } else if(day == "일") {
                    cell += '<div class="cell-content sun-style">';
                    cell += '<div class="day-number" sun-style>'+dayNumber+'</div>';
                } else {
		...
		obj._getMonthsHTML = function(startDate,endDate) {
			...
			for (var index = 1; (date.isBefore(endDate) || date.isSame(endDate)) ; index++){
				...
				cell += '<div class="date-formatted"><span class="month-name">'+fullYear+ '.' +'</span> '+monthNumber+'</span></div>';
				cell += '</div>';	
