    ...
    $.dataGrid = function(selector, settings){
        ...
        var grid={
    		...
    		appendRow2 : function(rows){
    			...
    			var rowArrayLength = _this.rowArray.length; 
    			var partRowArray = _this.rowArray[(rowArrayLength-1)]; //마지막 로우배열을 꺼내서..
    			var newArr = _this.convertToArray( tempTbody.get(0).rows, 0 ); 
    			
    			//tbody block이 꽉찬경우
    			if(partRowArray.length == config.paging.rows){
    				_this.rowArray[rowArrayLength] = newArr;
    				_this.rowArray.length = rowArrayLength + 1;
    				if(_this.cellFixMode){
    					_this.tbodyTableFix.append('<tbody id="tbody_"'+rowArrayLength+'>').append($(rows).clone());
    				}
    				_this.tbodyTable.append('<tbody id="tbody_"'+rowArrayLength+'>').append(rows);
    			}else{
    				partRowArray.push(newArr[0]);
    				_this.rowArray[(rowArrayLength-1)] = partRowArray;
    				if(_this.cellFixMode){
    					$("tbody[id=\"tbody_"+(rowArrayLength-1)+"\"]",_this.tbodyTableFix).append($(rows).clone());
    				}
    				$("tbody[id=\"tbody_"+(rowArrayLength-1)+"\"]",_this.tbodyTable).append(rows);
    			...
    			var scrollHeight = _this.tableBodyDiv.attr('scrollHeight');
    			_this.tableBodyDiv.scrollTop(_this.tableBodyDiv.attr('scrollHeight')+300);
    		...
    		removeRow : function(row){
    			...
    			rowObj.each(function(){
    				...
    				if(execute!="insert"){
    					...
    					_this.editedRowArray["remove"].push(_this.rowArray[tbodySeq].splice(index,1)[0]);
    				}
