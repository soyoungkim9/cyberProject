    ...
    $.dataGrid = function(selector, settings){
        ...
        var grid={
            ...
            _initLayout : function(){
        		...
        		if( $("col[class*=\"fixable\"]", this.theadColGroup).size() > 0 ){
	        		...
	        		_this.tbodyTable.delegate("tbody tr","mouseover mouseout",function(event){
	        			...
	        			var rowIndex 	= $("tr",tbody).index(tr);
            ...
            _syncWithFix : function(mode,jsonData){
            	if(_this.cellFixMode == true){
            		...
            		if("addSelectedTR" == mode){
                		if(isFixTable){
                			$("TBODY#tbody_"+info.tbodySeq+" TR",_this.tbodyTable).eq(info.rowIndex).addClass(config.selectClassName);
                		}else{
                			$("TBODY#tbodyFix_"+info.tbodySeq+" TR",_this.tbodyTableFix).eq(info.rowIndex).addClass(config.selectClassName);
                		}
            		} else if("removeSelectedTR" == mode){
                		if(isFixTable){
                			$("TBODY#tbody_"+info.tbodySeq+" TR",_this.tbodyTable).eq(info.rowIndex).removeClass(config.selectClassName);
                		}else{
                			$("TBODY#tbodyFix_"+info.tbodySeq+" TR",_this.tbodyTableFix).eq(info.rowIndex).removeClass(config.selectClassName);
                		}
                		...
                		if(isFixTable){
                			$("TBODY#tbody_"+info.tbodySeq+" TR",_this.tbodyTable).eq(info.rowIndex).replaceWith(info.row);
                		}else{
                			$("TBODY#tbodyFix_"+info.tbodySeq+" TR",_this.tbodyTableFix).eq(info.rowIndex).replaceWith(info.row);
                		}
    		...
    		_editMode:function(selTdObj){
            	...
            	if( col.hasClass("editable")){
					...
					var bindFunc = function(){
						...
						var isComeBackToOriginRowValue = false;
						...
						if(inputVal != selTdObj.attr("data-preValue")){
							...
							if("insert" != execute){ //기존에 존재하던 row라면
								...
								if($("TD[class=\"editedTdForUpdate\"]",selTrObj).size()==0){
									...
									if(selTrObj.attr("data-isEditedRow")=="true"){
										...
										isComeBackToOriginRowValue = true;
									}
								}
							}else{
						...
						if( selTrObj.attr("data-execute") !== undefined || isComeBackToOriginRowValue){
            ...
            _updateRowArray   : function(obj){
            	var targetRow = _this.getRowFromChild(obj);
            	var tbody = $(targetRow.get(0).parentNode);
				var tbodyId  = tbody.attr("id");
				var tbodySeq = tbodyId.substring(tbodyId.lastIndexOf("_")+1);//"tbody_" 또는 "tbodyFix_" 를 제거 
				var index = $("tr",tbody).index(targetRow);
				_this.rowArray[tbodySeq][index]=targetRow.clone().get(0);
				log("["+tbodySeq+","+index+"] "+targetRow.text(),"updateRowArray");
				
				var returnArray = new Object();
				returnArray["row"]      = obj.clone();
				returnArray["rowIndex"] = index;
    		...
    		_treeMode : function(){
    			...
    			for(var i in _this.rowArray){
					var rowArrayI = _this.rowArray[i];
					for( var j in rowArrayI){
						var targetTD	= $("TD",rowArrayI[j]);
						var parentValue = targetTD.eq(parentIndex).text();
						var selfValue 	= targetTD.eq(selfIndex).text();
						
						rowArrayI[j].childArray = new Array();
						rowArrayI[j].parentCode = parentValue;
						rowArrayI[j].selfCode   = selfValue;
						
						rootMap[selfValue] = rowArrayI[j];
    			...
    			var $expand = function(tr){
					...
					for( var i = (child.length-1), ic = child.length; i >= 0; i-- ){
						var childTR = child[i];
						if($(childTR).hasClass("shown")){
							$(childTR).show();
						}else{
							$(childTR).insertAfter(tr);
    			...
    			var $attachEventTree = function(tr,depth){
    				...
    				var span = $('<span class="tree_depth_'+depth+'" style="display:inline-block;width:18px;height:100%;">&nbsp;</span>');
    				var firstTD = $("td",tr).eq(0).append();
    				var originTDHtml = firstTD.html();
    				firstTD.html(span);
    				firstTD.append(originTDHtml);
    			...
    			_this._onSuccess(treeArray,'_treeMode',treeArray.length);
    		},
    		...
    		_onSuccess : function(data,textStatus,jqXHR,ajaxOptions){
    			...
    			if(config.tree!== undefined && textStatus!="_treeMode"){
    				return _this._treeMode();
    			}
    		...
    		appendRow : function(rows,haveToConfirm){
    			var tempTbody = $("<tbody>").append(rows);
    			var targetArray = _this.rowArray[0];
    		...
    		removeRow : function(row){
    			...
    			rowObj.each(function(){
    				...
    				var index = $("tr",tbody).index(this);
    				var removedRow = _this.rowArray[tbodySeq].splice(index,1)[0];
    				
    				if(execute!="insert"){
    					_this.editedRowArray["remove"].push(removedRow);
    				}
    		...
    		getRowsByCmd : function(cmd){
    			if(cmd=="delete"){
    				return $(_this.editedRowArray["remove"]);
    			}else{
    				var tempTbody = $("<tbody>");
    				for( var i in _this.rowArray){
    					var rowArrayI = _this.rowArray[i];
    					for( var j = 0,jc = rowArrayI.length; j < jc; j++ ){
    						var rowArrayIJ = $(rowArrayI[j]); 
    						if(rowArrayIJ.attr("data-execute")==cmd){
    							tempTbody.append(rowArrayIJ);
    						}
        				}
    				}
    				return $("tr",tempTbody);
    		...
    		unSelectRows : function(){
    			for(var i =0,ic = _this.selectedRows.length ; i < ic; i++ ){
					var selectedInfo = _this.selectedRows[i];
					var targetRow = $("TBODY#tbody_"+selectedInfo.tbodySeq+" TR",_this.tbodyTable).eq(selectedInfo.rowIndex);
					if(targetRow.size()==0){
						var selRow = $(_this.rowArray[selectedInfo.tbodySeq][selectedInfo.rowIndex]);
						selRow.removeClass(config.selectClassName);
						_this.rowArray[selectedInfo.tbodySeq][selectedInfo.rowIndex] = selRow.get(0);
					}else{
						var selRow = $("TBODY#tbody_"+selectedInfo.tbodySeq+" TR",_this.tbodyTable).eq(selectedInfo.rowIndex).removeClass(config.selectClassName);
						var info = _this._updateRowArray(selRow);
