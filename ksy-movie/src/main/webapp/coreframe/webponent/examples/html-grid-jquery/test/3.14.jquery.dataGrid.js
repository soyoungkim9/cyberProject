            ...
            _syncWithFix : function(mode,jsonData){
            	if(_this.cellFixMode == true){
            		if("addSelectedTR" == mode){
            			...
            			var rows    = jsonData.rows.clone();
            			var index   = jsonData.index;
            			var tbodyId = jsonData.tbodyId;
            			var tbodySeq= jsonData.tbodySeq;
            			if(tbodyId.indexOf("tbodyFix")==-1){	//fix테이블로 tr복사
            				$("TBODY#tbodyFix_"+tbodySeq+" TR",_this.tbodyTableFix).eq(index).replaceWith(rows);
            			}else{	//원테이블로 tr복사
            				$("TBODY#tbody_"+tbodySeq+" TR",_this.tbodyTable).eq(index).replaceWith(rows);
            			}
            	...
            	if( col.hasClass("editable")){
					...
					var bindFunc = function(){
						...
						//if(selTdObj.attr("data-isEditedValue")=="true"){
							var tbody = $(selTrObj.get(0).parentNode);
		    				var tbodyId  = tbody.attr("id");
		    				var tbodySeq = tbodyId.substring(tbodyId.lastIndexOf("_")+1);//"tbody_" 또는 "tbodyFix_" 를 제거 
	    					var index = $("tr",tbody).index(selTrObj);
		    				_this.rowArray[tbodySeq][index]=selTrObj.clone().get(0);
							
							_this._syncWithFix("editMode",{rows:selTrObj,tbodyId:tbodyId,index:index,tbodySeq:tbodySeq});
						//}
					};
