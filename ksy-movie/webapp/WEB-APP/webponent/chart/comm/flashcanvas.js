...
if (window["ActiveXObject"] && !window["CanvasRenderingContext2D"]) {
	...
	(function(window, document, undefined) {
		...
		var BASE_URL = config["swfPath"]
				|| getScriptUrl().replace(/[^\/]+$/, "");
		...
		"save", "restore", "scale", "rotate", "translate", "transform",
				"setTransform", "globalAlpha", "globalCompositeOperation",
				"strokeStyle", "fillStyle", "createLinearGradient",
				"createRadialGradient", "createPattern", "lineWidth",
				"lineCap", "lineJoin", "miterLimit", "shadowOffsetX",
				"shadowOffsetY", "shadowBlur", "shadowColor", "clearRect",
				"fillRect", "strokeRect", "beginPath", "closePath", "moveTo",
				"lineTo", "quadraticCurveTo", "bezierCurveTo", "arcTo", "rect",
				"arc", "fill", "stroke", "clip", "isPointInPath",
				// "drawFocusRing",
		...
		CanvasRenderingContext2D.prototype = {
			...
			createPattern : function(image, repetition) {
				...
				// If the second argument isn't one of the allowed values,
				// throws a
				...
				// If this is the first time to access the URL, the canvas
				// should be
...
//					++lock[canvasId];
				...
				if (this._font !== this.font) {
					try {
						...
						var font = [ style.fontStyle, style.fontWeight,
						             span.style.fontSize, style.fontFamily ].join(" ");
			...
			measureText : function(text) {
				...
				try {
					...
					// If this.font cannot be parsed as a CSS font value, then
					// it must
					// be ignored.
				}
			...
			drawImage : function(image, x1, y1, w1, h1, x2, y2, w2, h2) {
				...
				if (argc === 3) {
					...
					// If one of the sw or sh arguments is zero, the
					// implementation
			...
			// dy, [Optional] in float dirtyX, in float dirtyY, in float
			// dirtyWidth, in float dirtyHeight);
...
//					console.log("execute>>>"+commands);
		...
		function onFocus(event) {
			...
			try{
			if(!event.shiftKey){
				var each = function($canvas){
					var f = false;
					if($canvas.nextAll('a, input, button, select').length > 0){
						$canvas.nextAll('a, input, button, select').first().focus();
						f = true;
					}
					if($canvas.nextAll().find('a, input, button, select').length > 0){
						$canvas.nextAll().find('a, input, button, select').first().focus();
						f = true;
					}
					if(!f) {
				...
				var each = function($canvas){
					var f = false;
					if($canvas.prevAll('a, input, button, select').length > 0){
						$canvas.prevAll('a, input, button, select').last().focus();
						f = true;
					}
					if($canvas.prevAll().find('a, input, button, select').length > 0){
						$canvas.prevAll().find('a, input, button, select').last().focus();
						f = true;
					}
					if(!f) {
						$canvas = parent($canvas);
		...
		function onUnload() {
			...
			for ( var canvasId in canvases) {
				...
				// clean up the references of canvas.getContext and
				// canvas.toDataURL
				for (prop in canvas) {
		...
		var FlashCanvas = {
			initElement : function(canvas) {
				...
				var str = '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000"'
						...
						+ objectId
						+ '&stageWidth='+$(canvas).attr('width')+'&stageHeight='+$(canvas).attr('height')+'">'
			...
			resizeNoScale:function(a){a.firstChild.resizeNoScale()},
			...
			flush : function(canvasId) {
				...
				if(canvas.getContext('2d')._queue.length > 1000) {
					canvas.getContext('2d')._executeCommand();
				} 
		...
		// The width and height attributes of a canvas element must have values
		// that
