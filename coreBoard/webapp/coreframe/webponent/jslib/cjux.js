 ...
 * Copyright(c) 2006-2012 CyberImagination, Inc.
 * Licensed under the CyberImagination Commercial License V.2.2
 * 
...
var _viE_x = navigator.userAgent
...
Prototype.Browser.IE8 = Prototype.Browser.IE && !Prototype.Browser.IE6
...
var styleFloatNm;
if (document.all)
	styleFloatNm = "styleFloat";
else
	styleFloatNm = "cssFloat";
...
function popupLogMsg() {
	debugPopup = window
			.open(
					"",
					null,
					"height=170px,width=400px,top=0,left=0,scrollbars=yes,status=yes,toolbar=no,menubar=no,location=yes");
}
...
webponent.Layout.prototype = {
	initialize : function(mainPanelId, width, height, paddingSz) {
		...
		this.coverDiv = new Element(
				'div',
				{
					'style' : 'position:absolute;left:0px;top:0px;width:0px;height:0px;z-index:999999;'
							+ extSt
				});
	...
	addSplit : function(outlinePanel, mode, spliterParams) {
		...
		var resizable = false;
		if (spliterParams.resizable == true)
			resizable = true;
		// , splitWidth, isFix
		if (splitWidth == 0)
			return;
		...
		if (resizable) {
			...
			Event.observe(linePanel, 'mousedown', this.onSplitClickDn
					.bind(this));
		} else {
			s.cursor = '';
		}
	...
	onSplitClickDn : function(event) {
		...
		// Event.observe(this.parentPanel, 'mousemove',
		// this.onSplitMouseOver.bind(this));
		Event.observe(window.document, 'mousemove', this.onSplitMouseOver
				.bind(this));
		Event.observe(window.document, 'mouseup', this.onSplitClickUp
				.bind(this));
		...
		if (this.isMouseClick) {
			...
			if (this.verticalMode) {
				...
				if (this.minX < x && x < this.maxX)
					this.resizeSplitStyle.left = px(x);
			} else {
				var y = event.clientY;
				if (this.minY < y && y < this.maxY)
					this.resizeSplitStyle.top = px(y);
			}
		}
	},
	onSplitClickUp : function(event) {
		...
		if (this.isMouseClick) {
			...
			var x = 0;
			var y = 0;
			if (this.preObj.offsetWidth == 0 || this.nexObj.offsetWidth == 0
					|| this.preObj.offsetHeight == 0
					|| this.nexObj.offsetHeight == 0) {
				var sb = this.sideObj;
				x = sb.getAttribute('x');
				y = sb.getAttribute('y');
			...
			if (this.verticalMode) {
				...
				if (m1 == m2) {
					try {
						this.preObj.style.height = px(this.preObj.clientHeight
								+ y);
						this.nexObj.style.height = px(this.nexObj.clientHeight
								- y);
					} catch (e) {
						log('onSplitClickUp:' + this.preObj.clientHeight
								+ ' > ' + y);
					}
		...
		case webponent.Layout.CENTER:
			if (size) {
				if (size.indexOf("%") && Prototype.Browser.IE6) {
					...
					size = this.centerOutlineObj.offsetHeight * parseInt(size)
							/ 150;
					// alert(size);
					size = px(size);
				}
	...
	getCenterPos : function() {
		...
		for (k = 0; k < pl.length; k++) {
			var m = -1;
			if (pl[k].getAttribute)
				m = pl[k].getAttribute('mode');
			if (m == webponent.Layout.LEFT || m == webponent.Layout.RIGHT) {
				cwidth += pl[k].offsetWidth;

			} else if (m == webponent.Layout.TOP
					|| m == webponent.Layout.BOTTOM) {
	...
	resize : function() {
		...
		if (this.resizeX)
			s.width = pxPstv(pp.clientWidth - fixW);
		if (this.resizeY)
			s.height = pxPstv(pp.clientHeight - fixW);
		s.display = '';
		...
		if (this.bodyHeight > 30) {
			...
			for ( var i = 0, n = this.sideObjStyleArray.length; i < n; i++) {
				if (this.centerOutlineStyle != this.sideObjStyleArray[i])
					this.sideObjStyleArray[i].height = px(this.bodyHeight);
			}
		...
		if (this.centerOutlineObjArray.length > 0) {
			...
			for ( var i = 0, n = this.centerOutlineObjArray.length - 1; i < n; i++) {
			...
			if (lastBodyHeight > minHt) {
				...
				try {
					...
					pr.style.height = px(pr.clientHeight + lastBodyHeight
							- minHt);
					this.centerOutlineObjArray[this.centerOutlineObjArray.length - 1].style.height = minHt
							+ 'px';
				} catch (e) {
				}
			}
...
webponent.TabView.prototype = {
	...
	setTabBody : function() {
		if (!this.tabBody) {
			...
			// >>>'style':
			// 'overflow:auto;overflow-X:hidden;padding:0px;margin:0px;clear:both;border:0px
			// solid #00f;'
			});
	...
	add : function(label, panelId, param) {
		...
		if (!tab) {
			if (param.iframe == true) {
				var scroll = 'no';
				if (param.overflow == 'auto')
					scroll = 'auto';
				tab = new Element('iframe', {
					'id' : panelId,
					'scrolling' : scroll
				});
			} else
		...
		if (param.closable) {
			...
			// {'src':webponent.imgRootPath+'tab-close.gif', 'style':'margin:0px
			// 0px 0px 0px','idx':idx});
		...
		if (this.mouseOver)
			Event.observe(tabSel, 'mouseover', this.show.bind(this));
		else
			Event.observe(tabSel, 'mouseup', this.show.bind(this));
		return tabSel;
	},
	close : function(idx) {
	...
	show : function(idx) {
		...
		this.fixTop = findPosition(this.tabParent)[1]
				- findPosition(this.mp)[1];
	...
	resizing : function() {
		...
		try {
			...
			// log('tabBody size '+this.mp.offsetTop+ '
			// '+this.tabParent.clientHeight+ ' '+this.tabHeader.offsetHeight+ '
			// '+fixH);
			this.tabBody.style.height = px(this.tabParent.offsetHeight
					- this.tabHeader.offsetHeight + fixH);

		} catch (e) {
			log('tab resize err:' + e.message);
		}
...
webponent.DragDropProxy.prototype = {
	initialize : function(dragSource, dragNodeName) {
		...
		if (dragNodeName == null)
			dragNodeName = 'TABLE';
		if (this.dragSource.nodeName == dragNodeName)
			this.isSourceTable = true;
	},
	start : function() { // event regist;
		Event.observe(this.dragSource, 'mousedown', this.onMouseDownProxy
				.bind(this));
		Event.observe(window.document, 'mousemove', this.onMouseMoveProxy
				.bind(this));
		Event.observe(window.document, 'mouseover', this.onMouseOverProxy
				.bind(this));
		Event.observe(window.document, 'mouseup', this.onMouseUpProxy
				.bind(this));
	},
	...
	startDragPopup : function(src) {
		try {
			...
			if (this.isSourceTable) {
				...
				this.dragDiv.appendChild(src.cloneNode(true));
	...
	onMouseMoveProxy : function(e) {
		if (this.dragStart) {
			...
			this.dragDiv.style.top = e.clientY + 'px'; // log('webponent.DragDropProxy>
														// mouse move');
			this.onMouseMove(e);
		}
	},
	...
	onMouseOverProxy : function(e) { // log('xxxxxxxxx over');
		if (this.dragStart) {
			...
			if (row) {
				...
				if (this.dragSource == row.parentNode.parentNode)
					return;
				this.targetObj = row;
				this.targetObj.addClassName(FOCUS_CLASSNM);
			}
			this.onMouseOver(e); // log('webponent.DragDropProxy> mouse
									// over');
		}
	},
...
/*
 * var FOLDED = 'plus.gif'; var UNFOLDED = 'minus.gif'; var NO_CHILD =
 * 'blank.gif';
 */
...
webponent.HtmlGrid.prototype = {
	initialize : function(tblId, width, height, arg) {
		...
		if (this.arg.columnResizable == 'false'
				|| this.arg.columnResizable == false
				|| this.arg.autoColumnResizable)
		 ...
		 * //this.tblHeadOutline.width = px(tblWidth); } else { try { var
		 * cw=this.tblOutline.clientWidth; if(cw>this.scrollBarWidth) {
		 * if(Prototype.Browser.IE8) { this.tblHeadOutline.width = px(cw-
		 * this.scrollBarWidth+5);
		 * 
		 * }else { this.tblHeadOutline.width = px(cw- this.scrollBarWidth ); } }
		 * }catch(e) {} }
		 * 
		 */
	...
	_initFixTableHtml : function() {
		if (!this.tblCols) {
			...
			for ( var i = 0, i2 = this.tblHead.rows[0].cells, i3 = i2.length; i < i3; i++) {
	...
	_grd_th_sort : function(event) {
		...
		/*
		 * var d=parseInt(th.getAttribute('x')); var type='string'; try{ var
		 * typeClssNm = th.className; var i=typeClssNm.indexOf('Type');
		 * type=typeClssNm.substring(0,i);
		 * 
		 * }catch(e){} this.sortRow(d, 'dsc', type);
		 */
	},
	...
	_initSetTableHeadMeta : function() {
		...
		for ( var i = 0, i2 = this.tblHead.rows; i < i2.length; i++) {
			for ( var j = 0, j2 = i2[i].cells; j < j2.length; j++) {
				...
				j2[j].setAttribute('y', t.y); // alert('>>'+
												// this.hiddenColsArray[t.x].getAttribute("width")

				var el = $(j2[j]);
				if (el.hasClassName('stringType') || el.hasClassName('intType')
						|| el.hasClassName('dateType')
						|| el.hasClassName('extType1')
						|| el.hasClassName('extType2')
						|| el.hasClassName('extType3')) {
					...
					Event.observe(sortBtn, 'click', this._grd_th_sort
							.bind(this));
					Event.observe(sortBtn, 'focus', this._grd_th_onfcs
							.bind(this));
					Event.observe(sortBtn, 'blur', this._grd_th_onblur
							.bind(this));
				}
				...
				if (w && w > 0) {
					...
					// this.hiddenColsArray[t.x].setAttribute("width",
					// j2[j].offsetWidth );
				}
		...
		if (Prototype.Browser.IE8) {
			...
			for ( var i = 0; i < this.hiddenColsArray.length; i++) {
				...
				if (txtAlign == '')
					txtAlign = webponent.gridTextAlign;
			...
			for ( var i = 0, i2 = this.tblHead.rows; i < i2.length; i++) {
				for ( var j = 0, j2 = i2[i].cells; j < j2.length; j++) {
					...
					if (w && w > 0) {
						...
						// this.tblHeadCols.childNodes[t.x].setAttribute("width",
						// w);
	...
	_registEvent : function() {
		...
		Event.observe(window.document, 'mouseup', this.mouseUpSelfDrag
				.bind(this)); // event Column Sort
		this.columnSortBind = this.columnSort.bind(this);
		Event.observe(this.tblHead, 'mousedown', this.columnSortBind); // event
																		// Column
																		// Resize

		// multi header
		if (this.tblHead.rows.length < 5) {
			Event.observe(this.tblHead, 'mousemove', this.changeCursorState
					.bind(this));
			Event.observe(this.tblHead, 'mousedown', this.startColumnResize
					.bind(this));
		}
		...
		Event.observe(this.tblBodyOutline, 'scroll', this._onTblBodyScroll
				.bind(this)); // event regist
		// Event.observe(this.tblBodyOutline, 'scroll',
		// this._scrollVerticalEvnt); // event regist

		/*
		 * 0419 try { layoutEvent.regist(this.resizing.bind(this));
		 * gridEvent.regist(this.resizing.bind(this)); } catch(e) {}
		 */
	},
	...
	_scrollVertical : function() {
		...
		Event.stopObserving(this.tblBodyOutline, 'scroll',
				this._scrollVerticalEvnt);
		...
		Event.observe(this.tblBodyOutline, 'scroll', this._scrollVerticalEvnt); // event
																				// regist
	...
	_regTblReszEvnt : function(isActive) {
		...
		/*
		 * Event.observe(this.tblOutline.parentNode, 'propertychange',
		 * this.resizingEvent); Event.observe(this.tblOutline.parentNode,
		 * 'DOMAttrModified', this.resizingEvent);
		 * Event.observe(this.tblOutline.parentNode, 'DOMSubtreeModified',
		 * this.resizingEvent); Event.observe(this.tblOutline.parentNode,
		 * 'resize', this.resizingEvent);
		 */
		...
		if (isActive) {
			...
			Event
					.stopObserving(this.tbl, 'DOMNodeInserted',
							this.resizingEvent);
	...
	_initLayout : function() {
		...
		this.tblOutlineStyle = this.tblOutline.style; // this.tblOutline.onselectstart
														// = function(){return
														// false;};// there ???
		...
		if (this.tblCols) {
			...
			if (this.tblHead.rows.length > 0) {
				...
				this.tblHeadOutline
						.insertBefore(this.tblHeadCols, this.tblHead);
			}
				...
				if (h[i].nodeName == 'COL') {
					...
					if (h[i].style.display == 'none') {
						...
						if (!Prototype.Browser.oldIE)
							this.tblHead.rows[0].cells[idx].style.display = 'none';
			...
			// 0903
			/*
			 * for (var i = 0, m = this.hiddenColsArray.length; i < m; i++) { if
			 * (this.hiddenColsArray[i].style.width == '') {
			 * this.hiddenColsArray[i].style.width = this.tblOutline.clientWidth /
			 * m; // alert(this.hiddenColsArray[i].style.width ); } }
			 */
		...
		if (this.height) {
			...
			this.tblBodyOutline.style.overflowY = 'hidden';
		// if(Prototype.Browser.Gecko)
		// this.tbl.style.backgroundImage="url('"+webponent.imgRootPath+"table_body_bg_ff.jpg')";
		// else
		// this.tblBodyOutline.style.backgroundImage="url('"+webponent.imgRootPath+"table_body_bg.jpg')";
		 ...
		 * this.tbl.style.backgroundImage="url('"+webponent.imgRootPath+"table_body_bg_ff.jpg')";
		 * else
		 * this.tbl.style.backgroundImage="url('"+webponent.imgRootPath+"table_body_bg.jpg')";
		 */

		if (this.arg.useTableBG != false) {
			if (Prototype.Browser.IE6 || Prototype.Browser.IE7)
				this.tblBodyOutline
						.addClassName('tableBodyBackgoundForNotFirefox');
			else if (Prototype.Browser.Gecko)
				this.tbl.addClassName('tableBodyBackgoundForFirefox');
			else
				this.tbl.addClassName('tableBodyBackgoundForNotFirefox');
		}
		...
		if (!this.arg.isFixedWidth) {
			try {
				// var adjSize = 0;
				// if (Prototype.Browser.IE) adjSize = 1;
				// //this.tblHeadOutline.width=this.tblOutline.clientWidth-adjSize-this.scrollBarWidth;
			} catch (e) {
	...
	_initSortFunc : function() { // sort
		...
		if (Prototype.Browser.IE7 || Prototype.Browser.IE6) {
			this.sortImg = new Element(
					'div',
					{
						'style' : 'position:relative;top:3px;left:8px;margin:-15px -5px;float:left;vertical-align:top;',
						'class' : 'sortDescIcon'
					});
		} else {
			this.sortImg = new Element(
					...
					{
						// 'style':
						// 'position:relative;top:3px;left:8px;margin:0px
						// -5px;float:left;', 'class':'sortDescIcon'
						'style' : 'position:relative;top:3px;left:8px;margin:-15px -5px;float:left;vertical-align:top;',
						'class' : 'sortDescIcon'
					});
		}
	...
	toggleTblEdit : function(e) {
		var elmnt=e.element();
		this.editmode=elmnt.checked;
		
	},
	// 
	addSelRow : function() {
		var tr = document.createElement("TR");
		var c = this.tblCols.childNodes;
		var inputBox;
		var arrTd = [];
		for ( var j = 0; j < c.length; j++) {
			...
			if(this.levelIdx==j) {
				...
				if(this.tblBody.rows.length>0) {
					dTxt=this.tblBody.rows[this.tblBody.rows.length-1].cells[j].innerHTML;
				}
				arrTd[j].innerHTML = dTxt;
			}else if(this.titleIdx==j) {
				var imgObj = new Element('span', {
					'class' : NO_CHILD,
					'style' : 'margin-right:5px;margin-left:'
							+ this.getImgMargin(1) + 'px'
				});
				arrTd[j].appendChild(imgObj);
		    }else {
			  arrTd[j].innerHTML = '';
			}
		...
		if (outCondition != null) {
			if (outCondition instanceof Array) {
				for ( var i = 0; i < outCondition.size(); i++) {
					...
					Event.observe($(temp[1]), 'click', this.getData.bind(this,
							hash, content, arrTd, outCondition));
					if (sumField == true)
						content[j].observe(event, this.rowAction.bind(this));
				}
			} else {
				Event.observe($(outCondition), 'keyup', this.getData.bind(this,
						hash, content, arrTd, outCondition));
				if (sumField == true)
					content[j].observe(event, this.rowAction.bind(this));
			}
			...
			if (hash.isContain(j)) {
				...
				if (temp[0] == 'select') {
					...
					if (temp[2] == null)
						...
						content[j].value = temp[2];

					content[j].style.border = '0px';
					if ((this.tblBody.rows.length % 2) == 0)
						content[j].style.background = '#FFFFFF';
					else
						content[j].style.background = '#F8F8F6';
				...
				if (temp[3] == null)
					...
					content[j].name = temp[3] + '_' + this.tblBody.rows.length;

				arrTd[j].appendChild(content[j]);

				if (temp[0] != 'select' && temp[3] != null)
					arrTd[j].appendChild(label);

				Event.observe(content[j], event, this.getData.bind(this, hash,
						content, arrTd, outCondition));
				if (sumField == true)
					content[j].observe(event, this.rowAction.bind(this));
	...
	getData : function(hash, content, arrTd, outCondition) {
		...
		for ( var i = 0; i < hash.size(); i++) {
			...
			if (key_type == 'radio') {
				...
				values.putValue(key[0], eval("$F(content[hash[" + i
						+ "].key].id)"));
			}
		}
	...
	updateSumValue : function(gridObj, arr) {
		...
		tmpDiv.innerHTML = '<table><tbody id=' + this.tblBody.id + '>'
				+ tr.innerHTML + '</tbody></table>';
	...
	_updateTableBodyFail : function(transport) {
		...
		try {
			var errMsg = 'server process fail. reponse code:'
					+ transport.status;
			if (this.onDataError) {
	...
	updateBodyFromGrid : function(gridObj) {
		...
		tmpDiv.innerHTML = '<table><tbody id=' + this.tblBody.id + '>'
				+ gridObj.tblBody.innerHTML + '</tbody></table>';
	...
	columnCtrlVisible : function() {
		...
		this.columnChkDiv.style.left = px(this.getPosition(this.popupDiv)[0]
				+ this.popupDiv.offsetWidth);
		// this.columnChkDiv.style.top = px( this.getPosition(this.popupDiv)[1]
		// + this.popupDiv.offsetHeight - 20 );
		this.columnChkDiv.style.top = px(this.getPosition(this.popupDiv)[1]);
	...
	_columnCtrl : function() {
		...
		for ( var i = 0; i < c.length; i++) {
			...
			Event.observe(this.headCheckBox[i], 'mouseup', this.columnVisible
					.bind(this));
			tDiv.appendChild(this.headCheckBox[i]);
			if (parseInt(c[i].rowSpan) != this.tblHead.rows.length)
				this.headCheckBox[i].disabled = true;
	...
	doColumnVisible : function(idx) {
		...
		if (!el) {
			...
			// }
			if (Prototype.Browser.IE)
				this.headCheckBox[idx].checked = false;
		} else {
			...
			if (this.tbl.width != '100%') { // this.tblStyle.width = tbSz +
											// this.arrTdSz[idx];
				// this.tblHeadOutline.style.width = tbSz + this.arrTdSz[idx];
			}
			if (Prototype.Browser.IE)
	...
	excelDownLoad : function(url) {
		if (typeof (url) != 'string')
			url = '/coreframe/transform';
		// var tblContent = this.tbl.innerHTML;
		// var tblContent = this.getXmlText({skipTableTag: true});
		var tblContent = '<thead>' + get_xhtml(this.tblHead)
				+ '</thead><tbody>' + get_xhtml(this.tblBody) + '</tbody>';
	...
	columnSort : function(e, el) { // alert(e.isLeftClick());
		...
		var type = null;
		if (el.hasClassName('stringType'))
			type = 'string';
		else if (el.hasClassName('intType'))
			type = 'int';
		else if (el.hasClassName('dateType'))
			type = 'date';
		else if (el.hasClassName('extType1'))
			type = 'ext1';
		else if (el.hasClassName('extType2'))
			type = 'ext2';
		else if (el.hasClassName('extType3'))
			type = 'ext3';
		...
		if (type != null) {
			...
			/*
			 * var tArr = new Array(); var tblHeadRows = this.tblHead.rows; if
			 * (tblHeadRows.length > 1) { headIdx =
			 * el.getAttribute('ext-columnIndex'); } else { // headIdx =
			 * el.cellIndex; headIdx = el.getAttribute('x'); }
			 */
			...
			if (type == 's')
				this.servSortRow(el, headIdx, this.sortType);
			else
				this.sortRow(headIdx, this.sortType, type);
	...
	_fixSortImgPosition : function(el) {
		...
		try {
			...
			s.display = 'block';
			/*
			 * if($(el).visible()) s.display='block'; else s.display='none';
			 */
	...
	sortRow : function(idx, sortType, type) {
		...
		if (type == 'int') {
			...
			colArr.sort(sortDate);
		else if (type == 'string')
			colArr.sort(sortStr);
		else if (type == 'ext1')
			colArr.sort(this.sortExt1);
		else if (type == 'ext2')
			colArr.sort(this.sortExt2);
		else if (type == 'ext3')
			colArr.sort(this.sortExt3);
		...
		if (Prototype.Browser.IE) {
			if (sortType == 'dsc') {
				...
				for ( var i = 0, k = colArr.length; i < k; i++) {
					var r = rowArr[colArr[i].o - 1];
					this.tblBody.appendChild(r);
				}
			}
	...
	_chkResizeArea : function(e) {
		...
		if (Prototype.Browser.IE) {
			...
			var check = e.clientX
					+ (((!Prototype.Browser.IE) && (el.tagName == "DIV")) ? el.offsetLeft
							: 0) + scrLeft;
			btsz = size + (pos - check);
		}
	...
	startColumnResize : function(e) {
		...
		if (el.tagName == "TH" && el.style.cursor != "default") {
			...
			this.tDoResize = this.doResize.bindAsEventListener(this, el, pos,
					size, el2, size2, tblwidth);
	...
	stopResize : function(e) {
		...
		Event.stopObserving(this.tblHead, 'mousedown', this.columnSortBind);
		Event.observe(this.tblHead, 'mousedown', this.columnSortBind); // this.tblHead
																		// =
																		// null;
		this.tblHead.onmouseup = '';
		document.body.onmouseup = '';
		this.tStopResize = null; // >> this.tblHiddenHead.style.display =
									// 'none';
		...
		for ( var j = 0, m = this.tblHead.rows.length; j < m; j++) {
					...
					if (rw == 1) {
						if (gx > rx && gx <= (rx + rw)) {
							...
							if (gx == rx && cw == rw) {
								...
								var colSz2 = parseInt(c.offsetWidth) + gap; // var
																			// tblWSz
																			// =
																			// e.clientX
																			// -
								...
								// c.style.width= '';
								// c.setAttribute('width',px(colSz2 < 15 ? 15 :
								// colSz2));
							}
							break;
						}
			...
			if (j == 0) {
				...
				try {
					...
					this.tblHeadCols.childNodes[colIdx].setAttribute('width',
							colSize);
	...
	reset : function() {
		log('webponent.HtmlGrid> reset'); // this.tblBody.style.display =
											// 'none';
		if (this.onreset) {
	...
	resetComplete : function() {
		...
		if (Prototype.Browser.Gecko) {
			...
			for ( var i = 0, n = r.length; i < n; i++) {
				var cr = r[i];
				for ( var j = 0, k = this.hiddenColsIdxArray.length; j < k; j++) {
		...
		if (this.isTreeMode) {
			this.setTreeTableMode(this.levelIdx, this.titleIdx,
							this.isDragable);
		} // this.resizing();
	...
	initFooter : function() {
		...
		if (footer && !this.footDiv) {
			...
			if (outlineNext)
				this.tblOutline.parentNode.insertBefore(this.footDiv,
						outlineNext);
			else
				this.tblOutline.parentNode.appendChild(this.footDiv);
			...
			this.appendFootMenu(' ADD ROW', this.addSelRow.bind(this)); // ??????
			this.appendFootMenu(' Direct edit mode', this.toggleTblEdit.bind(this), 'checkbox'); // ??????
			this.footDiv.appendChild(new Element('span')
					.update(' &nbsp;&nbsp; '));
		}
	},
	appendFootMenu : function(label, func, typ) {
		if(!typ) typ="button";
		...
		if(typ=='checkbox') {
			...
			this.footDiv.appendChild(span);
			Event.observe(btn, 'change', func);
		}else {
			this.footDiv.appendChild(btn);
		...
		for ( var i = 0; i < len; i++) {
			...
			if (depth <= baseLevel) {
				...
				if (selImgObj.className && selImgObj.className != NO_CHILD) {
					// if (depth == baseLevel) selImgObj.src = webponent.imgPath
					// + FOLDED;
					if (depth == baseLevel)
						selImgObj.className = FOLDED;
					// else selImgObj.src = webponent.imgPath + UNFOLDED;
					else
						selImgObj.className = UNFOLDED;
				}
			} else {
	...
	_initPopupMenu : function() {
		...
		Event
				.observe(window.document, 'click', this.onRightMouseDn2
						.bind(this)); // Event.observe(this.tbl, 'mouseup',
										// this.onRightMouseUp.bind(this) );
	...
	deleteAllRow : function(eventOrForce) {
		if (eventOrForce == true
				|| window.confirm('Do you really delete all of row?')) {
			var f;
			if (this.ondelete) {
				f = this.ondelete.bind(this);
			}
	...
	setTreeTableMode : function(lvlIdx, tleIdx, isDragable) {
		...
		if (this.isDragable) {
			Event.observe(this.tbl, 'mouseover', this.mouseOvSelfDrag
					.bind(this)); // Event.observe(b, 'mousemove',
									// this.mouseMvSelfDrag.bind(this) );
			// Event.observe(document, 'selectstart', onDisableSelect );
			document.onselectstart = onDisableSelect;
		}
		this.initFooter();
	...
	processTitle : function(cell, depth, nexCell, nextDepth, isDragable) {
		if (cell == null)
			return; // TD in < span > & bnsp; < / span > < img > text 3 type
		...
		if(lbl && (lbl.hasClassName(FOLDED) || lbl.hasClassName(UNFOLDED) ||lbl.hasClassName(NO_CHILD)) ) {
			if (depth < nextDepth) {
				lbl.className=UNFOLDED;
			}else {
				lbl.className=NO_CHILD;
			}
			lbl.style.marginLeft= this.getImgMargin(depth)+'px';
			return;
	...
	selectRow : function(selTr, isMultiSelection, callEvent, e) {
		...
		if (this.seqSelectMode && this.selRowArray.length > 0) {
			...
			if ((baseIdx - p.rowIndex) < 0) {
				isTopDown = false; // p =
									// this.selRowArray[this.selRowArray.length
									// - 1];
			}
			var tmp = new Array();
			tmp[0] = p;
		...
		if (isMultiSelection && row.hasClassName(this.selectClssNm)) {
			...
			var tmp = new Array();
			for ( var k = 0, n = this.selRowArray.length; k < n; k++) {
				if (this.selRowArray[k].rowIndex != row.rowIndex)
					tmp[tmp.length] = this.selRowArray[k];
			}
			this.selRowArray = tmp;
			return;
		}
		...
		if (this.isTreeMode) {
			...
			for ( var i = row.rowIndex + 1, n = this.tbl.rows.length; i < n; i++) {
				var r = this.tbl.rows[i];
				var lvl =0;
				try{
				lvl = r.cells[this.levelIdx].innerHTML;
				}catch(e) {
					break;
				}
	...
	moveRow : function(targetRow) {
		...
		if (targetRow && this.selRowArray.length > 0) { // target rowIndex
			...
			if (rowIdx < stx || rowIdx > etx) {
				...
				if (!targetLvl)
					targetLvl = 0;
				this.moveSelectRow(targetRow.next(), true, targetLvl); // remove
																		// folder
																		// image
				if (this.noChildRow) {
	...
	_changeTitleImg : function(row, mode) {
		try {
			// row.cells[this.titleIdx].firstChild.src = webponent.imgPath +
			// mode;
	...
	moveSelectRow : function(be, ischangeLevel, targetLvl) {
		var baseLvl = 0;
		if (ischangeLevel)
			baseLvl = this.getLevel(this.selRowArray[0]);
	...
	unselectRow : function(isRemoveFocusOnly) {
		if (this.selRow && this.selRowArray.length > 0) {
			...
			for ( var j = 1, n = this.selRowArray.length; j < n; j++) {
				this.selRowArray[j].removeClassName(this.selectClssNm);
				this.selRowArray[j].removeClassName('child-selected'); // log("remove
																		// >>>>>>>>>>
																		// " +
																		// this.selRowArray[j].rowIndex);
			}
			this.selRowArray = new Array();
		}
	},
	...
	mouseOvSelfDrag : function(e) { // log('grid mouse over');
		if (this.dragDiv) { // this.dragDiv.style.display = 'block';
			...
			if (nodeNm != 'TD' && nodeNm != 'TH')
				return; // var ps = findPosition(srcElmnt);
	...
	mouseDnSelfDrag : function(e, isForce) {
		...
		if ($(headerCell).hasClassName('editable') == 'true')
			return;
		...
		if (!this.dragDiv) {
			...
			if (this.titleIdx >= 0) {
				...
				this.dragDiv.update(' [' + (this.selRowArray.length) + '] '
						+ txt);
				document.body.appendChild(this.dragDiv);
				try {
				...
				divS.left = p[0] + 10 + "px";
				divS.top = e.clientY + "px"; // divS.width = c.clientWidth +
	...
	processFoldering : function(event, imgObj, parentView) {
		...
		for ( var i = rowIdx + 1; i < rowLen; i++) {
			...
			if (parentView == HIDDEN) {
				...
				var selImgObj = r.cells[this.titleIdx].firstChild; // window.status
																	// = '>>>>'
																	// +
																	// selImgObj.nodeName;
				if (selImgObj.className == FOLDED) {
					if (depth == (selDepth - 1))
						this.processFoldering(event, selImgObj, parentView);
				}
			}
			}catch(e) {
				break;
		...
		if (img != null)
			imgObj.className = img; // window.status = '>>>>>>' +
									// selImgObj.nodeName + ' > ' + rowLen;
	},
	getColumnHeader : function(td) {
		for ( var m = this.tblHead.rows, i = m.length - 1; i >= 0; i--) {
			for ( var k = 0, k1 = m[i].cells; k < k1.length; k++) {
				if (k1[k].getAttribute('x') == td.cellIndex)
					return k1[k];
			}
		}
	},
	outEditMode : function() {
		...
		if (this.onFinishEdit && this.preInputObj != null) {
			...
			this.onFinishEdit.bind(this)(this.preInputObj.value.trim(),
					this.selInputObj.getAttribute("preValue"),
					this.selInputObj, this.preTrObj);
		}
	...
	onTblClick : function(e) { // log('webponent.HtmlGrid> onTblClick');
		...
		var nodeNm = selTdObj.nodeName; // window.status = '>>' +
										// selTdObj.nodeName;
		...
		this.outEditMode();
		if (nodeNm == 'SPAN'
				&& (selTdObj.className == FOLDED || selTdObj.className == UNFOLDED)) {
		...
		if (nodeNm == 'TD') {
			if ((selTdObj.firstChild != null) || selTdObj.firstChild == null) {
				...
				if (headerCell == selTdObj)
					return;
				var editable = (this.editmode && $(headerCell).hasClassName('editable') );
				...
				if (Prototype.Browser.IE)
					h = px(selTdObj.clientHeight - 4);
				var value = selTdObj.innerHTML; // if(value == '&nbsp;')
												// alert('tt');
				var newHTML = "";
				var newHTMLType = "";
				...
				if (optVal != null) {
					...
					for ( var j = 0, n = optValues.length; j < n; j++) {
						var selChk = "";
						if (value == optValues[j])
							selChk = "selected";
						newHTML = newHTML + "<" + "option value='"
								+ optValues[j] + "' " + selChk + " >"
								+ optValues[j] + "<" + "/option>";
					}
					...
					newHTML = "<input type=text value='"
							+ value
							+ "' style='vertical-align:bottom;margin:0px;padding:1px 2px;width:"
							+ w + ";height:" + h
				...
				try {
					this.preInputObj.observe('keydown', this.onTblKeyDn
							.bind(this));

					// editable ???? ???? ??????? TD?? ???? ???????
					// isEditedValue??? ????? ??????? true ???? ????
					if (newHTMLType == "input") {
						this.selInputObj.observe('keyup',
								this._checkValueChanged.bind(this,
										this.selInputObj, this.preInputObj)); // input
					} else if (newHTMLType == "select") {
						this.preInputObj.observe('click',
								this._checkValueChanged.bind(this,
										this.selInputObj, this.preInputObj)); // select
					}
					// -------------------------------------------------------------------------------------------
				} catch (e) {
				}
	...
	_getCellIndex : function(cell) {
		...
		for ( var y = 0; y <= rIndex; y++) {
			...
			for ( var x = 0; x <= numCols; x++) {
				table[y] = (table[y] || []); // assign dimension && prevent
												// overflow
				r = rows[y];
				if (!table[y][x]) {
					...
					for ( var i = 0, l = (c.rowSpan || 1); i < l; i++) {
						for ( var j = 0, m = (c.colSpan || 1); j < m; j++) {
							table[y + i] = (table[y + i] || []); // assign
																	// dimension
																	// &&
																	// prevent
							// overflow
							table[y + i][x + j] = 1; // debug with:
														// '['+x+'/'+y+']'
							if (c === cell) { // we've found our cell, no more
												// work is needed
								// so we use an tried and tested tactic of early
								// return
								return {
									x : (x + j),
									y : (y + i)
	...
	resizing : function(checkCellIdx, tblRefHead) {
		...
		if (this.tblOutline.offsetWidth == 0)
			return;

		this.isResizing = true;

		log(this.tbl.id + ' webponent.HtmlGrid > resizing '
				+ this.tblOutlineStyle.width + ':'
				+ this.tblOutline.offsetWidth + ' ' + this.preTableWidth);
		...
		if (this.isFullHeight) {
			...
			try {
				if (true) {
					...
					this.tblOutline.style.height = parseInt(p.parentNode.clientHeight)
							- ipt - ipb + 'px';
		...
		if (this.height
				&& (this.tblOutline.clientHeight - this.tblHeadOutline.clientHeight) > 10) {
			this.tblBodyOutline.style.height = px(this.tblOutline.clientHeight
					- this.tblHeadDiv.offsetHeight);
		}

		/*
		 * if (this.relativeWidth) { try { var g=this.tblOutline.parentNode;
		 * 
		 * if(g.clientWidth>0) { this.tblOutline.style.width = '1px'; var
		 * g1=g.style; var l=parseInt(g1.paddingLeft); var r=0; if(l>=0) {
		 * r=parseInt(g1.paddingRight); } else { l=10; r=10; }
		 * 
		 * this.tblOutline.style.width = g.clientWidth - l-r+'px'; } }catch(e) {}
		 *  }
		 */
		...
		try {
			...
			for ( var rowSeq = 0, mm = this.tblHead.rows.length; rowSeq < mm; rowSeq++) {
				...
				for ( var i = 0, m = headCells.length; i < m; i++) { // log(rowSeq+':'+i+'
																		// check');
					...
					if (rowSeq == 0 && i == checkCellIdx) { // alert(viewHeadCells[i].style.display);
						if (viewHeadCells[i].style.display == 'none') {
							// if (this.hiddenHeadColsArray[i].style.display ==
							// 'none') {
							...
							for ( var k = colIdx; k < mx; k++) {
								...
								if (!Prototype.Browser.oldIE) {
									...
									for ( var p = 0, p1 = this.tblBody.rows, p2 = p1.length; p < p2; p++) {
					...
					if (this.hiddenHeadColsArray[colIdx].style.display == 'none') {
						...
						if (Prototype.Browser.WebKit) {
							/*
							 * if(i==(m-1)) { viewHeadCells[colIdx].width =
							 * px(headCells[i].clientWidth+1); }else if(i==0) {
							 * viewHeadCells[colIdx].width =
							 * px(headCells[i].clientWidth-1); }else if(i==1) {
							 * viewHeadCells[colIdx].width =
							 * px(headCells[i].clientWidth-2); }else
							 */
							 ...
							 * viewHeadCells[colIdx].style.width =
							 * px(headCells[i].offsetWidth-1); }else
							 */
							{
	...
	_onTblBodyScroll : function(e) {
		...
		this.tblHeadDiv.style.marginLeft = px(parseInt(this.tblBodyOutline.scrollLeft)
				* -1);
		this._fixSortImgPosition();
	...
	getXmlText : function(param) {
		if (!param)
			param = '';
		var str = '';
		if (param.skipTableTag != true)
			str += "<table border='1'>";
		str += "<thead>";
		for ( var i = 0, r = this.tblHead.rows; i < r.length; i++) {
			str += "<tr>";
			for ( var j = 0, c = r[i].cells; j < c.length; j++) {
				if (c[j].style.display != 'none') {
					str += "<th width='" + c[j].offsetWidth + "'";
					if (c[j].colSpan > 1)
						str += " colspan='" + c[j].colSpan + "'";
					if (c[j].rowSpan > 1)
						str += " rowspan='" + c[j].rowSpan + "'";
		...
		str += "</thead>";
		str += "<tbody>";
		for ( var i = 0, r = this.tblBody.rows; i < r.length; i++) {
			str += "<tr>";
			for ( var j = 0, c = r[i].cells; j < c.length; j++) {
...
webponent.History = {
	initialize : function(options) {
		...
		if (Prototype.Browser.IE) {
			this.locator = new webponent.History.Iframe('ajaxHistoryHandler',
...
Number.prototype.format = function() {
	return this.str().reverse().replace(/(\d{3})(?=\d)(?!\d+\.)/g, '$1,')
			.reverse();
};
